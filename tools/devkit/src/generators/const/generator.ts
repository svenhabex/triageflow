...
type NormalizedSchema = ConstGeneratorSchema & BaseNormaliedSchemaType;

function normalizeOptions(tree: Tree, options: ConstGeneratorSchema): NormalizedSchema {
  const fileName = names(options.name).fileName;
  const name = fileName === 'const' ? fileName : `const-${names(options.name).fileName}`;
  ...
  const parsedTags = ['type:const', `domain:${names(options.domain).fileName}`];
...
export default async function (tree: Tree, options: ConstGeneratorSchema): Promise<void> {
